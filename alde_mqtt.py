#!/usr/bin/env python3
"""
Alde 3030 Plus Climate - MQTT Bridge v17
================================
"""
...
def sniff(data, direction="RX"):
    if not DEBUG_SNIFF or not data:
        return
...
# ── MQTT ─────────────────────────────────────────────────────────────────────
MQTT_HOST   = '100.15.150.150' #hier deinen Mqtt host
MQTT_PORT   = 1883             # Standard Port
MQTT_USER   = ''               #falls username, hier setzen
MQTT_PASS   = ''	       #falls password, hier setzen
...
DISPLAY_DISC  = 'homeassistant/switch/alde_3030/alde_display/config'
DISPLAY_STATE = 'alde/display/state'
CMD_DISPLAY   = 'alde/display/cmd'

CMD_SYSTEM_OFF  = 'alde/system/cmd'
SYSTEM_OFF_DISC = 'homeassistant/button/alde_3030/alde_system_off/config'
...
hot_water_temp = None
external_start_state = "OFF"
# External Start Relais (Victron) – Topic anpassen oder auf None setzen zum Deaktivieren
EXTERNAL_START_TOPIC = "W/deinvictronidausvrm/system/0/Relay/1/State"
EXTERNAL_START_ON    = '{"value": 1}'
EXTERNAL_START_OFF   = '{"value": 0}'
# ── LIN helpers ──────────────────────────────────────────────────────────────
...
def read_state(ser):
    ...
    sniff(raw, direction="RX")
    leftover = ser.read(ser.in_waiting or 0)
    if leftover:
        sniff(leftover, direction="RX-EXTRA")
    if len(raw) >= 12:
        response = raw[3:12]
        data = response[:8]
        cs = response[8]
        if cs == lin_checksum_enhanced(ID_INFO, data):
            ...
            if b5 == 0x6B:
                ...
                ac_mode_status = "alde off"
            else:
                mode_bits = b5 & 0b11100110
                if mode_bits in (0x60, 0x61):
                    ac_mode_status = "alde auto"
                elif mode_bits in (0x20, 0x21):
                    ac_mode_status = "manual"
                else:
                    ac_mode_status = f"unknown_{b5:02X}"
            return {
...
def build_payload(state, new_setpoint=None, new_gas=None, new_elec=None, new_water=None, 
    ...
    # b5 aufbauen (Basis aus aktuellem Status)
    b5 = state['raw_b5']

    # Water Mode nur ändern, wenn explizit übergeben
    ...
    # Klima-Modus setzen
    if new_ac_mode is not None:
        water_bits  = b5 & 0x18
        display_bit = b5 & 0x01

        if new_ac_mode == "alde off":
            b5 = 0x60          # ← hier
        else:
            mode_map = {
                "alde auto": 0x60,
            ...
            if new_ac_mode in mode_map:
                b5 = mode_map[new_ac_mode] | water_bits | display_bit
...
def publish_discovery(client):
    ...
    client.publish("homeassistant/select/alde_3030/alde_klima_modus/config", json.dumps({
        "name": "Alde Klima Modus",
        "unique_id": "alde_3030_ac_mode",
        "object_id": "alde_klima_modus",
        "state_topic": CLIMATE_STATE,
        "value_template": "{{ value_json.ac_mode }}",
        "command_topic": "alde/climate/cmd/ac_mode",
        "options": ["alde auto", "manual"],
        "availability_topic": AVAIL_TOPIC,
        "device": DEVICE
    }), retain=True)

    client.publish(DISPLAY_DISC, json.dumps({
        "name": "Display",
        "unique_id": "alde_3030_display",
        "object_id": "alde_display",
        "state_topic": DISPLAY_STATE,
        "command_topic": CMD_DISPLAY,
        "payload_on": "ON",
        "payload_off": "OFF",
        "state_on": "ON",
        "state_off": "OFF",
    ...
    client.publish(SYSTEM_OFF_DISC, json.dumps({
        "name": "System aus",
        "unique_id": "alde_3030_system_off",
        "object_id": "alde_system_off",
        "command_topic": CMD_SYSTEM_OFF,
        "payload_press": "OFF",
        "availability_topic": AVAIL_TOPIC,
        "device": DEVICE,
        "icon": "mdi:power",
...
def publish_state(client, state):
    ...
    client.publish(CLIMATE_ATTR, json.dumps({
        ...
        'ac_mode_status':      state['ac_mode']  # NEU
    }), retain=True)
    ...
    client.publish(DISPLAY_STATE, 'ON' if state.get('panel_on') else 'OFF', retain=True)
...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        client.subscribe("alde/climate/cmd/ac_mode")
        client.subscribe("alde/red/hot_water_temp")
        client.subscribe("alde/red/external_start")
        client.subscribe(CMD_DISPLAY)
        client.subscribe(CMD_SYSTEM_OFF)
...
def on_message(client, userdata, msg):
    global pending_cmd, hot_water_temp, external_start_state
    topic = msg.topic
    ...
    with cmd_lock:
        ...
        if topic == "alde/climate/cmd/ac_mode":
            mode = payload.lower()
            if mode in ('alde auto', 'manual'):
                pending_cmd['ac_mode'] = mode

        elif topic == CMD_SYSTEM_OFF:
            pending_cmd['ac_mode'] = 'alde off'
            pending_cmd['water'] = 'off'
            pending_cmd['gas'] = 0
            pending_cmd['elec'] = 0
            pending_cmd['display'] = False
            if EXTERNAL_START_TOPIC:
                client.publish(EXTERNAL_START_TOPIC, EXTERNAL_START_OFF)
            print("[CMD] System aus → alles OFF")

        elif topic == CMD_TEMP:
            try:
                pending_cmd['setpoint'] = float(payload)
            except:
                print(f"[MQTT] Invalid temperature: {payload}")

        elif topic == CMD_MODE:
            if payload == 'heat':
                pending_cmd['gas'] = 1
                if EXTERNAL_START_TOPIC:
                    client.publish(EXTERNAL_START_TOPIC, EXTERNAL_START_ON)
            elif payload == 'off':
                pending_cmd['gas'] = 0
                # kein Relais OFF
            ...
            if payload in elec_map:
                ...
                if elec_map[payload] >= 1 and EXTERNAL_START_TOPIC:
                    client.publish(EXTERNAL_START_TOPIC, EXTERNAL_START_ON)
                # bei Off: kein Relais OFF

        elif topic == CMD_DISPLAY:
            if payload.upper() == 'ON':
                pending_cmd['display'] = True
                pending_cmd['gas'] = 1
                if EXTERNAL_START_TOPIC:
                    client.publish(EXTERNAL_START_TOPIC, EXTERNAL_START_ON)
                print("[SCHUTZ] Display ON → Gas ON")
            ...
            if payload in WATER_OPTIONS:
                ...
                if payload == 'boost' and EXTERNAL_START_TOPIC:
                    client.publish(EXTERNAL_START_TOPIC, EXTERNAL_START_ON)
        ...
        elif topic == "alde/red/hot_water_temp":
            try:
                hot_water_temp = float(payload)
            except:
                pass

        elif topic == "alde/red/external_start":
            external_start_state = payload.strip().upper()
# ── Main ──────────────────────────────────────────────────────────────────────
...
def main():
    ...
    last_published = (
        ...
        initial_state.get('ac_mode'),
        initial_state.get('panel_on'),
            ...
            if cmd:
                ...
                if state:
                    payload = build_payload(state,
                        ...
                        new_ac_mode  = cmd.get('ac_mode'),
                        new_display  = cmd.get('display'))

                    print(f"[CMD] {cmd}")
                    send_control(ser, payload)

                    # Schnelles Nachpoll'en (wie im Original)
                    for _ in range(5):
                        time.sleep(0.3)
                        new_state = read_state(ser)
                        if new_state:
                            with state_lock:
                                current_state = new_state
                            key = (
                                new_state['zone1_temp'], new_state['setpoint'],
                                new_state['gas_active'], new_state['electric_kw'],
                                new_state['pump_running'], new_state['water_mode'],
                                new_state['outdoor_temp'], new_state['error'],
                                new_state['valve_open'], new_state['ac_input'],
                                new_state.get('ac_mode'),
                                new_state.get('panel_on'),
                            )
                            if key != last_published:
                                publish_state(client, new_state)
                                last_published = key
                                print(f"[CMD CONFIRM] sp={new_state['setpoint']}°C "
                                      f"gas={new_state['gas_active']} "
                                      f"elec={new_state['electric_kw']}kW "
                                      f"water={new_state['water_mode']}")
                                break
...
# ── Schutzregel ──────────────────────────────────────
                if (state.get('panel_on') and
                    state['water_mode'] in ('on', 'boost') and
                    hot_water_temp is not None and hot_water_temp < 43 and
                    state['electric_kw'] <= 1 and
                    state['gas_active'] == 0):
                    print(f"[SCHUTZ] Display an + Water={state['water_mode']}, WW={hot_water_temp}°C → Gas ON")
                    payload = build_payload(state, new_gas=1)
                    send_control(ser, payload)
